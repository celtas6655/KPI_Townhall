<!DOCTYPE html>
<?php require_once 'daneNowyTest.php';?>
...
//                                    },
                            ...
                            chart1.render();
                            function renderArrow(chart1) {
  for (var i = 0; i < chart1.data[0].dataPoints.length; i++) {
    if (chart1.data[0].dataPoints[i].enableArrow) {
      var axisBounds = chart1.axisX[0].get("bounds");
      var x1, x2, y1, y2;
      var threshold = chart1.axisY[0].stripLines[0].value;

      x1 = x2 = chart1.axisX[0].convertValueToPixel(chart1.data[0].dataPoints[i].x);
      y1 = axisBounds.y1;
      y2 = chart1.axisY[0].convertValueToPixel(chart1.data[0].dataPoints[i].y);

      chart1.ctx.strokeStyle = "#66ff33";
      chart1.ctx.moveTo(x1, y1);
      chart1.ctx.lineTo(x2, y2);
      chart1.ctx.stroke();

      if (chart1.data[0].dataPoints[i].y < threshold) {
        chart1.ctx.moveTo(x2, y2);
        chart1.ctx.lineTo(x2 + 10, y2 + 10);
        chart1.ctx.moveTo(x2, y2);
        chart1.ctx.lineTo(x2 - 10, y2 + 10);
        chart1.ctx.stroke();
      } else {
        chart1.ctx.moveTo(x1, y1);
        chart1.ctx.lineTo(x1 + 10, y1 - 10);
        chart1.ctx.moveTo(x1, y1);
        chart1.ctx.lineTo(x1 - 10, y1 - 10);
        chart1.ctx.stroke();
      }
